...
def make_loan(request, account_number, pay_back=False):
    ...
    if request.method == 'POST':
        ...
        return render(
            request, 'account_management_app/account_details.html', {
                'account': my_account,
        ...
        return render(
            request, 'account_management_app/loan.html', {
...
def transfer(request, account_number):
    ...
    try:
        my_account.make_payment(Decimal(request.POST['amount']),
                                request.POST['account_number'])
...
def saving_account_transfer(request, account_number):
    customer = request.user.customer
    saving_account = Account.objects.get(account_number=account_number,
                                         is_saving_account=True)
    context = {}
    if request.method == 'POST':
        ...
        if transfer_account_number:
            ...
            return render(
                request, 'account_management_app/saving_account_detail.html', {
                    'account': saving_account,
                    'message':
                    'Please select an amount and account to transfer to'
                })
        transfer_account = Account.objects.get(
            account_number=transfer_account_number)
        all_accounts = Account.objects.filter(user=request.user,
                                              is_saving_account=False)

        try:
            transfer_account.make_payment(amount,
...
def chatbot_messages(request):
    ...
    if (request.method == 'POST'):
        ...
        return HttpResponseRedirect(
            reverse('account_management_app:chatbot_conv'))
    else:
        return HttpResponseRedirect(
            reverse('account_management_app:chatbot_conv'))
...
def external_transfer(request, account_number):
    ...
    if request.method == 'POST':
        try:
            external_bank_account = Account.objects.get(
                bank=request.POST['bank_id'])
            amount = Decimal(request.POST['amount'])
            # Make local reservation to external bank
            sender_account = Account.objects.get(account_number=account_number)
            sender_account.make_payment(amount,
                                        external_bank_account.account_number)
            context['total_balance'] = customer.total_balance_bank_accounts
            ExternalLedgerMetadata.objects.create(
                status='pending',
                token=uuid.uuid4(),
                reservation_bank_account=external_bank_account,
                amount=amount,
