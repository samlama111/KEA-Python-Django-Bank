...
class Account(models.Model):
    ...
    def create(user, name=None, is_saving_account=False):
        ...
        new_account = Account(user=user,
                              bank=local_bank,
                              is_saving_account=is_saving_account)
        new_account.save()

    def get_transactions(self):
        my_transactions = Ledger.objects.filter(
            account=self).order_by('-created_timestamp')
        return my_transactions

    def get_loan_transactions(self):
        loan_transactions = Ledger.objects.filter(
            account=self, is_loan=True).order_by('-created_timestamp')
    ...
    def get_amount_owed(self):
        ...
        if loan_transactions.count() == 0:
            return 0
        my_loan_transactions_with_balance = loan_transactions.aggregate(
            balance=Sum("amount"))
        return my_loan_transactions_with_balance['balance']

    def get_saving_account_transactions(self):
        saving_account_transactions = Ledger.objects.filter(
            account=self,
            is_saving_account=True).order_by('-created_timestamp')
    ...
    def saving_account_balance(self):
        ...
        if saving_transaction.count() == 0:
            return 0
        saving_transaction_with_balance = saving_transaction.aggregate(
            balance=Sum("amount"))
    ...
    def balance(self):
        ...
        if my_transactions.count() == 0:
            return 0
        my_transactions_with_balance = my_transactions.aggregate(
            balance=Sum("amount"))
        ...
        if is_loan == True and hasattr(user, 'customer'):
            ...
            if customer_rank == 'basic':
                raise ValidationError(
                    'Cant apply for loan. Please upgrade your user rank.')
    ...
    def make_payment(self,
        ...
        with transaction.atomic():
            ...
            Ledger.objects.create(account=target_account,
                                  is_creditor=True,
                                  amount=amount,
                                  ...
                                  variable_symbol='',
                                  is_saving_account=is_saving_account)
            Ledger.objects.create(account=self,
                                  is_creditor=False,
                                  amount=-amount,
                                  transaction_id=transaction_id,
                                  note='',
...
class ExternalLedgerMetadata(models.Model):
    ...
    reservation_bank_account = models.ForeignKey(Account,
